        ...
        for stat in self.stats:
            ...
            for iter_num in range(1, self.iters + 1):
                try:
                    ...
                    print(f"Could not open '{current_path}'")
                    sys.exit(1)
                except KeyError as e:
                    print(f"No key in JSON: {e}")
                except Exception as e:
...
def run(args):
    plotter_initial = PlotterJsonExtractor(path=args.file)
    ...
    for ds, name in zip(plotter_initial.ds, plotter_initial.display_ds):
        ...
        for val in plotter_initial.xvalues:
            path_to_result = f"{plotter_initial.out_folder}/{ds}_{plotter_initial.xtitle}{val}_aggregated.json"
