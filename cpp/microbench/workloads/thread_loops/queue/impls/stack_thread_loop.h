...
struct StackThreadLoopBuilder : public ThreadLoopBuilder {
    ...
    StackThreadLoopBuilder* set_ins_ratio(double ins_ratio) {
        parameters.INS_RATIO = ins_ratio;
        return this;
    }

    StackThreadLoopBuilder* set_rem_ratio(double del_ratio) {
        parameters.REM_RATIO = del_ratio;
        return this;
    }

    StackThreadLoopBuilder* set_rq_ratio(double rq_ratio) {
        parameters.RQ_RATIO = rq_ratio;
        return this;
    }

    StackThreadLoopBuilder* set_args_generator_builder(
        ArgsGeneratorBuilder* args_generator_builder) {
        argsGeneratorBuilder = args_generator_builder;
        return this;
    }

    StackThreadLoopBuilder* init(int range) override {
    ...
    ThreadLoop* build(globals_t* g, Random64& rng, size_t thread_id,
        ...
        return new StackThreadLoop(g, rng, thread_id, stop_condition, this->RQ_RANGE,
                                     argsGeneratorBuilder->build(rng), parameters);
    }

    void to_json(nlohmann::json& json) const override {
        json["ClassName"] = "StackThreadLoopBuilder";
    ...
    ~StackThreadLoopBuilder() override {
