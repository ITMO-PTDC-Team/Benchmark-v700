...
class TemporaryOperationThreadLoop : public ThreadLoop {
    PAD;
    std::vector<std::array<double, 4>> cdf_;
    Random64& rng_;
    PAD;
    ...
    std::vector<size_t> stages_durations_;
    PAD;
...
public:
    TemporaryOperationThreadLoop(globals_t* g, Random64& rng, size_t thread_id,
                                 ...
                                 size_t stages_number, std::vector<size_t> stages_durations,
                                 std::vector<RatioThreadLoopParameters> const& ratios,
        ...
        : ThreadLoop(g, thread_id, std::move(stop_condition), rq_range),
          ...
          time_(0),
          pointer_(0) {
        cdf_.resize(stages_number);
        stages_durations_ = std::move(stages_durations);
...
struct TemporaryOperationsThreadLoopBuilder : public ThreadLoopBuilder {
    ...
    TemporaryOperationsThreadLoopBuilder& set_stages_number(const size_t stages_number) {
        ...
        ratios.resize(stages_number);
        stagesDurations.resize(stages_number);
    ...
    TemporaryOperationsThreadLoopBuilder& set_stages_durations(
        std::vector<size_t> stages_durations) {
        stagesDurations = std::move(stages_durations);
        return *this;
    }
    ...
    void from_json(const nlohmann::json& j) override {
        ...
        std::copy(std::begin(j["stagesDurations"]), std::end(j["stagesDurations"]),
                  stagesDurations.begin());

        size_t i = 0;
        for (const auto& j_i : j["ratios"]) {
            ratios[i] = RatioThreadLoopParameters(j_i);
            ++i;
        }
    ...
    ~TemporaryOperationsThreadLoopBuilder() override = default;
};
